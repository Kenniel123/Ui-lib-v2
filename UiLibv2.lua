...
local c = {
    ...
    Notification = {
        ...
        Timer = Color3.fromRGB(99, 138, 255)
    },
    Info = Color3.fromRGB(99, 138, 255),
...
local ThemePresets = {
    ...
    Midnight = {
        ...
        Secondary = Color3.fromRGB(18, 22, 45),
        Border = Color3.fromRGB(40, 48, 85),
        Text = Color3.fromRGB(230, 235, 255),
        TextDark = Color3.fromRGB(110, 120, 160),
        Accent = Color3.fromRGB(100, 140, 255)
    },
    Ocean = {
        Background = Color3.fromRGB(8, 20, 25),
        Secondary = Color3.fromRGB(14, 32, 38),
        Border = Color3.fromRGB(35, 60, 70),
        Text = Color3.fromRGB(210, 245, 255),
        TextDark = Color3.fromRGB(90, 140, 160),
    ...
    Emerald = {
        ...
        Secondary = Color3.fromRGB(16, 32, 24),
        Border = Color3.fromRGB(35, 60, 45),
        Text = Color3.fromRGB(220, 255, 240),
        TextDark = Color3.fromRGB(90, 150, 110),
        Accent = Color3.fromRGB(60, 230, 140)
    },
    Rose = {
        Background = Color3.fromRGB(22, 12, 17),
        Secondary = Color3.fromRGB(35, 20, 28),
        Border = Color3.fromRGB(65, 35, 50),
        Text = Color3.fromRGB(255, 230, 245),
        TextDark = Color3.fromRGB(160, 100, 130),
        Accent = Color3.fromRGB(255, 100, 180)
    },
    Cyber = {
        Background = Color3.fromRGB(12, 14, 18),
        Secondary = Color3.fromRGB(20, 24, 30),
        Border = Color3.fromRGB(48, 56, 68),
...
function AcrylicBlur:_CreateRoot()
    ...
    self._mesh = CreateInstance("SpecialMesh", {
...
function AcrylicBlur:_Render(distance)
    ...
    local function Update()
        ...
        if self._mesh then self._mesh.Scale = Vector3.new(width, height, 0) end
    end
...
function Library:SetTheme(presetName)
    local theme = ThemePresets[presetName]
    if not theme then
        self:Notify({ Title = "Error", Description = "Unknown theme: " .. tostring(presetName), Duration = 2 })
        return
    end
    ...
    -- Recursively recolor every element in the GUI
    local function recolor(parent)
        for _, child in ipairs(parent:GetChildren()) do
            if child:IsA("Frame") then
                if child.Name == "Container" then
                    CreateTween(child, { BackgroundColor3 = c.Background }, animationspeed.Normal)
                elseif child.Name == "Header" or child.Name == "Separator" or child.Name == "Line" or child.Name == "LineLeft" or child.Name == "LineRight" then
                    child.BackgroundColor3 = c.Border
                elseif child.Name == "Dot" then
                    -- skip dots, they have custom colors
                elseif child.BackgroundTransparency < 0.5 and child.BackgroundColor3 ~= Color3.new(0,0,0) then
                    CreateTween(child, { BackgroundColor3 = c.Secondary }, animationspeed.Normal)
                end
            elseif child:IsA("TextLabel") then
                if child.Name == "TabText" or child.Name == "Label" or child.Name == "DividerText" then
                    CreateTween(child, { TextColor3 = c.TextDark }, animationspeed.Normal)
                elseif child.Name == "Name" or child.Name == "Title" then
                    CreateTween(child, { TextColor3 = c.Text }, animationspeed.Normal)
                elseif child.Name == "Value" or child.Name == "Description" or child.Name == "Content" then
                    -- skip custom-colored values
                end
            elseif child:IsA("TextBox") then
                CreateTween(child, { TextColor3 = c.Text, PlaceholderColor3 = c.TextDark }, animationspeed.Normal)
            elseif child:IsA("ImageLabel") or child:IsA("ImageButton") then
                if child.Name == "Icon" or child.Name == "Arrow" or child.Name == "Hamburger" then
                    CreateTween(child, { ImageColor3 = c.TextDark }, animationspeed.Normal)
                end
            elseif child:IsA("UIStroke") then
                child.Color = c.Border
            elseif child:IsA("ScrollingFrame") then
                -- recurse into scrolling frames
            end
            recolor(child)
        end
    end
    ...
    if self.screenGui then
        recolor(self.screenGui)
    end
    
    -- Update title specifically
    if self.titleLabel then
        CreateTween(self.titleLabel, { TextColor3 = c.Text }, animationspeed.Normal)
    end
    
    -- Update current tab highlight
    if self.currentTab and self.currentTab.textLabel then
        self.currentTab.textLabel.TextColor3 = c.Text
    end
    if self.currentTab and self.currentTab.icon then
        self.currentTab.icon.ImageColor3 = c.Text
    end
...
function Library:SetBackgroundImage(imageId, transparency)
    ...
    if not imageId or imageId == "" then
        -- Restore original container opacity
        if self.container then
            CreateTween(self.container, { BackgroundTransparency = 0.05 }, animationspeed.Normal)
        end
    ...
    local resolvedImage = imageId
    
    -- If it's a web URL (not rbxassetid), download and convert via executor asset system
    if imageId:match("^https?://") then
        local success, result = pcall(function()
            -- Download image data
            local imageData = game:HttpGet(imageId)
            
            -- Generate a unique temp filename
            local fileName = "AcrylicBG_" .. tostring(math.random(100000, 999999)) .. ".png"
            ...
            -- Convert to usable asset via executor functions
    ...
    -- Make container background more transparent so image shows through
    if self.container then
        CreateTween(self.container, { BackgroundTransparency = 0.6 }, animationspeed.Normal)
    end
    
    self._bgImage = CreateInstance("ImageLabel", {
    ...
    CreateCorner(self._bgImage, 5)
    
    -- Store the original URL for opacity changes
    self._bgImageUrl = imageId
end
...
function Library:_CreateMainv0rtexd()
    ...
    CreateStroke(self.container, c.Border, 0.8)
...
function Library:_CreateContentArea()
    ...
    self.tabsContainer = CreateInstance("ScrollingFrame", {
        Name = "TabsContainer",
    ...
    CreateListLayout(self.tabsContainer, 4, Enum.SortOrder.LayoutOrder)
    CreatePadding(self.tabsContainer, 8, 8, 8, 8)
    ...
    CreatePadding(searchContainer, 5, 3, 7, 7)
    ...
    CreateStroke(searchBox, c.Border, 0.6)
    CreatePadding(searchBox, 0, 0, 10, 10)
    ...
    self._searchResultsFrame = CreateInstance("ScrollingFrame", {
        Name = "SearchResults",
    ...
    CreateListLayout(self._searchResultsFrame, 6, Enum.SortOrder.LayoutOrder)
    CreatePadding(self._searchResultsFrame, 10, 10, 15, 15)

    -- No results label
    self._noResultsLabel = CreateInstance("TextLabel", {
        Name = "NoResults",
        FontFace = f.Regular,
        TextColor3 = c.TextDark,
        Text = "No matching components found",
        BackgroundTransparency = 1,
        TextSize = textsize.Normal,
        Size = UDim2.new(1, 0, 0, 40),
        Visible = false,
        Parent = self._searchResultsFrame
    })
    ...
    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        if query == "" then
            ...
            self._searchActive = false
            self._searchResultsFrame.Visible = false
            self.contentContainer.Visible = true
            -- Clear search result clones
            for _, child in ipairs(self._searchResultsFrame:GetChildren()) do
                if child:IsA("Frame") then
                    child:Destroy()
                end
            end
            self._noResultsLabel.Visible = false
            return
        end

        self._searchActive = true
        self._searchResultsFrame.Visible = true
        ...
        local matchCount = 0
        for _, elem in ipairs(self._allElements) do
            if elem.name:lower():find(query, 1, true) then
                matchCount = matchCount + 1
                -- Create a result entry
                local resultFrame = CreateInstance("Frame", {
                    Name = "Result_" .. elem.name,
                    BackgroundColor3 = c.Secondary,
                    BackgroundTransparency = 0.4,
                    BorderSizePixel = 0,
                ...
                CreateCorner(resultFrame, 5)
                CreateStroke(resultFrame)
                ...
                local tabInfo = CreateInstance("TextLabel", {
                    Name = "TabInfo",
                    FontFace = f.Regular,
                    TextColor3 = c.TextDark,
                    Text = elem.tabName .. " › " .. elem.type,
                ...
                local jumpBtn = CreateInstance("TextButton", {
                    Name = "Jump",
                    Text = "",
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 1, 0),
                ...
                jumpBtn.MouseButton1Click:Connect(function()
                    -- Clear search and jump to tab
                    searchBox.Text = ""
                    if elem.selectTab then
                        elem.selectTab()
                    end
                end)

                jumpBtn.MouseEnter:Connect(function()
                    CreateTween(resultFrame, {BackgroundTransparency = 0.2}, animationspeed.Fast)
                end)
                jumpBtn.MouseLeave:Connect(function()
                    CreateTween(resultFrame, {BackgroundTransparency = 0.4}, animationspeed.Fast)
                end)
            end
        end

        self._noResultsLabel.Visible = (matchCount == 0)
    end)
    ...
    CreatePadding(self.contentContainer, 12, 12, 18, 18)
end
...
function Library:CreateTab(name, icon)
    local tab = {
        name = name,
        icon = icon or "rbxassetid://112235310154264",
        elements = {},
        _library = self
    }
    ...
    local tabStroke = CreateStroke(tabBtn, c.Border, 0.7)
    ...
    CreatePadding(tab.content, 12, 12, 16, 16)

    clickBtn.MouseButton1Click:Connect(function()
        Library._SelectTab(self, tab, tabBtn, tabStroke, iconLabel, tabText)
    end)

    clickBtn.MouseEnter:Connect(function()
        if self.currentTab ~= tab then
            CreateTween(tabBtn, {BackgroundTransparency = 0.85}, animationspeed.Fast)
        end
    end)

    clickBtn.MouseLeave:Connect(function()
        if self.currentTab ~= tab then
            CreateTween(tabBtn, {BackgroundTransparency = 0.95}, animationspeed.Fast)
        end
    end)
    ...
    function tabMethods:CreateKeybind(config)
        ...
        return Library._CreateKeybind(self, config, self._library)
    end
...
function Library._SelectTab(lib, tab, btn, stroke, icon, textLabel)
    
    if lib.currentTab then
        lib.currentTab.content.Visible = false
        CreateTween(lib.currentTab.button, {BackgroundTransparency = 0.95}, animationspeed.Fast)
    ...
    CreateTween(btn, {BackgroundTransparency = 0.8}, animationspeed.Fast)
...
function Library._CreateButton(tab, config)
    ...
    button.MouseEnter:Connect(function()
        CreateTween(frame, {BackgroundTransparency = 0.3}, animationspeed.Fast)
        CreateTween(icon, {ImageColor3 = c.Text}, animationspeed.Fast)
    end)

    button.MouseLeave:Connect(function()
        CreateTween(frame, {BackgroundTransparency = 0.5}, animationspeed.Fast)
    ...
    button.MouseButton1Click:Connect(function()
        ...
        CreateTween(frame, {BackgroundColor3 = c.SurfaceHover}, animationspeed.Fast)
        task.wait(0.1)
        CreateTween(frame, {BackgroundTransparency = 0.5}, animationspeed.Fast)
...
function Library._CreateLabel(tab, config)
    ...
    local color = config.Color or c.Text
    ...
    return {
        ...
        SetColor = function(_, newColor)
            CreateTween(valueLabel, { TextColor3 = newColor }, animationspeed.Fast)
            CreateTween(dot, { BackgroundColor3 = newColor }, animationspeed.Fast)
        end,
...
function Library._CreateDivider(tab, text)
    local frame = CreateInstance("Frame", {
        Name = "Divider",
        BackgroundTransparency = 1,
        Size = UDim2.new(1, 0, 0, (text and text ~= "") and 22 or 12),
        Parent = tab.content
    })
